...
import { Stethoscope, MoreHorizontal, Trash2 } from 'lucide-react'
...
function filterClinicas(items: ClinicaItem[], tab: Tab): ClinicaItem[] {
...
export function ClinicasTable({
  ...
  const visible = filterClinicas(clinics, activeTab)
  ...
  async function handleBulkDelete() {
    ...
    if (!confirm(`¿Eliminar ${ids.length} clínica${ids.length !== 1 ? 's' : ''}? Esta acción no se puede deshacer.`)) return
    setDeleting(true)
    try {
      await Promise.all(
        ids.map(id =>
          fetch(`/api/admin/clinics/${id}`, { method: 'DELETE' }).then(r => {
            if (!r.ok) throw new Error(`Error al eliminar clínica ${id}`)
          })
        )
      )
      toast.success(`${ids.length} clínica${ids.length !== 1 ? 's' : ''} eliminada${ids.length !== 1 ? 's' : ''}.`)
      setSelected(new Set())
      router.refresh()
    } catch {
      toast.error('Error al eliminar las clínicas seleccionadas.')
    } finally {
      setDeleting(false)
    }
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        {TABS.map(tab => {
          ...
          const count = filterClinicas(clinics, tab.key).length
          return (
            <button
              key={tab.key}
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-card overflow-hidden">
        {visible.length === 0 ? (
          <EmptyState
            icon={Stethoscope}
            title="Sin clínicas"
            description="No hay clínicas en esta categoría."
            size="sm"
          />
        ) : (
            ...
            <table className="w-full">
              <thead className="bg-gray-50/60">
                <tr className="border-b border-gray-100">
                  ...
                  {showEmpresa && (
                    <th className="text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wide px-4 py-3 hidden sm:table-cell">Empresa</th>
                  )}
                  <th className="text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wide px-4 py-3 hidden md:table-cell">Código</th>
